...
public:
    ...
    std::map<string, shared_ptr<Symbol>> types;     // type name string of vars
...
//            cout << "(*it)->locals[" << name << "] = " << (*it)->locals[name] << endl;
    ...
    shared_ptr<Symbol> getSymbolType(string name) const{
        for(auto it=blockStack.rbegin(); it!=blockStack.rend(); it++){
//            cout << "(*it)->locals[" << name << "] = " << (*it)->locals[name] << endl;
    ...
    void setSymbolType(string name, shared_ptr<Symbol> value){
